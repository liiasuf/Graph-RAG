...
from rag_system.e2e import run_e2e

MAIN_PATH = os.getenv("MAIN_PATH", str(ROOT_PATH))

CONFIG_PATH = str(ROOT_PATH / "configs" / "e2e_hotpotqa.yaml")
...
class HotpotQARAGRunner:
    """
    A runner for the HotpotQA RAG baseline
    """
    ...
    def _find_latest_metrics_path(self):
        """
        Find the most recent 'metrics.json' inside the experiments directory
        """
        latest_path = None
        latest_mtime = -1.0
    ...
    def run(self):
        """
        Run the full HotpotQA RAG baseline and register metrics in history.
        """

        print(f"Running HotpotQA RAG baseline with config: {self.config_path}")
        run_e2e(self.config_path)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run HotpotQA RAG end-to-end")

    parser.add_argument(
        "--config",
        type=str,
        default=None,
        help="Path to YAML config (default: configs/e2e_hotpotqa.yaml)",
    )
    ...
    runner = HotpotQARAGRunner(config_path=config_path)
    runner.run()
